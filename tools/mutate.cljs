(ns mutate
  ...
  ## Three deliberate differences from the `kotoba-lang/iso3166` copy

  This started as that copy byte-for-byte (md5 a06eda8c269efc3b7676ee04baad1ff1)
  — the interrupt-safe one. Everything above is unchanged. Three things had to
  differ here, and each is a difference this repository forces, not a
  preference:

  1. THE BASELINE IS NOT GREEN, AND CANNOT BE. This library reads its data
     from sibling west checkouts (see `kotoba-actors.config`), and two of
     them — com-etzhayyim-kadode and com-etzhayyim-tsugite — have no `data/`
     tracked in git at all. 12 assertions error on missing files before any
     mutation is applied, identically under both runtimes. The upstream copy
     exits 2 on a red baseline, which here would mean never measuring
     anything. So the baseline's failing-test set is RECORDED, and a mutation
     scores as killed only when it reddens something that was NOT already red.
     A mutation whose only effect is on the already-broken tests is a
     SURVIVOR, exactly as if the suite had not noticed at all.

  2. THE RUNTIME IS A PARAMETER. `MUTATE_CMD` selects the suite; it defaults
     to `clojure -M:test -m cognitect.test-runner`. The point of this repo's change was portability, so
     a harness that could only drive the JVM would leave the `:cljs` half of
     every reader conditional unmeasured — which is precisely the survivor
     `kotoba-lang/technology` and `kotoba-lang/iso3166` both had to record
     rather than fix. Here the table is run twice and both results reported.

  3. `maxBuffer` IS RAISED to 64 MiB. `execSync`'s 1 MiB default SIGTERMs the
     child when a mutation produces a lot of output — and under nbb a single
     errored assertion prints a full sci callstack, several KiB each, so a
     mutation that reddens most of the suite blows past 1 MiB easily. A
     SIGTERMed child is read by the code above as \"the process group was
     signalled\" and ABORTS THE WHOLE TABLE, so the cap does not just lose one
     row, it loses every row after it."
...
(def ^:private test-cmd
  (or (some-> (aget js/process "env") (aget "MUTATE_CMD"))
      ;; This repo's `:test` alias declares `:exec-fn`, not `:main-opts`, so a
      ;; bare `clojure -M:test` opens a REPL and measures nothing. Naming the
      ;; runner is not a style choice here.
      "clojure -M:test -m cognitect.test-runner"))

(def ^:private exec-opts
  ;; See difference 3 in the namespace docstring: the 1 MiB default turns a
  ;; noisy mutation into an aborted table.
  #js {:maxBuffer (* 64 1024 1024)})

(defn- run-tests []
  (try
    (let [out (.toString (cp/execSync (str test-cmd " 2>&1") exec-opts))]
      {:exit 0 :out out})
    (catch :default e
...
(defn- run-one [baseline-reds m]
  ...
  (try
    (let [{:keys [exit out signal]} (run-tests)
          ...
          all-reds (failing-tests out)
          ;; Only NEW failures count. See difference 1 in the namespace
          ;; docstring: 12 assertions are already red on missing seed files,
          ;; and scoring those as kills would let every mutation pass.
          reds (vec (remove baseline-reds all-reds))
      ...
      (cond
        ...
        (empty? reds)
        (do (println (str "    SURVIVOR — no test noticed that was not already"
                          " red at baseline."
                          (when (seq all-reds)
                            (str " (" (count all-reds)
...
(defn -main [& args]
  ...
  (let [table (edn/read-string (.readFileSync fs "tools/mutations.edn" "utf8"))
    ...
    (when-let [sig (:signal baseline)]
      ...
      (js/process.exit (signal-exit-code sig)))
    (when-not (re-find #"Ran \d+ tests" (:out baseline))
      ;; No summary line means the suite never ran. Mutating from there would
      ;; score every row against nothing.
      (println "baseline: the suite did not run — refusing to mutate.")
      (println (:out baseline))
      (js/process.exit 2))
    (let [baseline-reds (set (failing-tests (:out baseline)))]
      (println "baseline:" (summary-line (:out baseline))
               (str "[" test-cmd "]")
               (if (empty? baseline-reds)
                 "GREEN"
                 (str "RED in " (count baseline-reds)
                      " tests — these are EXCLUDED from scoring:")))
      (doseq [t (sort baseline-reds)] (println (str "  - " t)))
      (when (empty? ms)
        (println "no mutations selected — refusing to report a pass")
        (js/process.exit 2))
    (let [results (mapv (partial run-one baseline-reds) ms)
